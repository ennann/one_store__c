 ...
 */
 module.exports = async function (params, context, logger) {
  ...
  const getTaskBatchNo = async (object_task_def) =>{
     await faas.function('GetTaskBatchNumber').invoke({object_task_def: object_task_def}).then((res)=>{
 ...
 let finalTaskDefList = [];
  ...
  let all_tasks = await fetchTaskDefRecords(query,"一次性非立即发布任务",logger);
     finalTaskDefList.push(...all_tasks);
  //满足条件的记录数>0
  if (finalTaskDefList.length > 0){
      //组装 aPaaS 数据，创建任务处理记录
      // 根据满足条件的记录数准备批量创建任务处理记录
      const batchCreateData =[]
      finalTaskDefList.forEach(item =>{
          let taskBatchNo = getTaskBatchNo(item);
          logger.info("taskBatchNo--->",taskBatchNo);
          batchCreateData.push({
              task_def: { _id: item._id }, // 任务定义
              batch_no: item._id  + '-' + '000001', //任务批次号
              option_status: 'option_01',//任务状态（创建）
              task_create_time:  dayjs().valueOf()//任务创建时间
          });
      })
      ...
      logger.info('创建任务处理记录成功-->', JSON.stringify(batchCreateSuccess, null, 2));
      logger.info('创建任务处理记录失败-->', JSON.stringify(batchCreateFailed, null, 2));
